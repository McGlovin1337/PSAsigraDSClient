    ...
    sealed public class SetDSClientMSSqlServerBackupSet: BaseDSClientMSSqlServerBackupSet
    {
        ...
        protected override void ProcessMsSqlBackupSet()
        {
            ...
            if (backupSet != null)
            {
                ...
                {
                    Win32FS_Generic_BackupSetCredentials dbCredentials = Win32FS_Generic_BackupSetCredentials.from(DbCredential.GetCredentials());

                    if (ShouldProcess($"Backup Set: '{backupSetName}'", $"Update Database Credentials with Username: '{DbCredential.UserName}'"))
